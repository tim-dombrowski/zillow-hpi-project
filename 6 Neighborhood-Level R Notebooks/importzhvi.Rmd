---
title: "Neighborhood-Level ZHVI Data Import and Cleaning"
...
zhviurl = "https://files.zillowstatic.com/research/public_csvs/zhvi/Neighborhood_zhvi_uc_sfrcondo_tier_0.33_0.67_sm_sa_month.csv"
```
...
```{r rawsave}
write_csv(ZHVIraw, paste0(rawdir,"ZHVI_raw_neighborhood.csv"))
```
...
Let's summarize the distribution of cities by state. *Hawaii has the fewest neighborhoods with just five. Then all other states + DC have at least a dozen neighborhoods.*
...
```{r wide2long}
ZHVI_Neighborhood = pivot_longer(ZHVIraw,
                        cols=10:ncol(ZHVIraw),
                        names_to="Date",
                        values_to="ZHVI")
# Format date variable as date)
ZHVI_Neighborhood$Date = as.Date(ZHVI_Neighborhood$Date)
```
...
dim(ZHVI_Neighborhood)
# Summarize each variable in the cleaned data frame
summary(ZHVI_Neighborhood)
# Display the first few rows
head(ZHVI_Neighborhood)
```
...
```{r dropcols}
ZHVI_Neighborhood = select(ZHVI_Neighborhood, -c(State, City, Metro, CountyName))
```
...
```{r cleansave}
write_fst(ZHVI_Neighborhood, paste0(cleandir,"ZHVI_clean_neighborhood.fst"))
```
